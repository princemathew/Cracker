function crack(){
    if(checkValue()){
        ...
        if(potfile.toString('utf8')){
            document.getElementById('result').innerHTML=potfile.toString('utf8')
            document.getElementById('cracksub').disabled = false;
                ...
                hashcat.stdout.on('data', function (data) {
                    document.getElementById('result').innerHTML = data.toString('utf8');
                    cracked ==1;
                });
                hashcat.on('close', () => {
                    document.getElementById('cracksub').disabled = false;
                    if(cracked ==1){
                        document.getElementById('result').innerHTML="keyspace exhausted"
                    }
                });
